...
mx = [fuzz.trimf(x_values_norm,
                 [x_means[i] - 0.3, x_means[i], x_means[i] + 0.3]) for i in range(6)]

my = [fuzz.trimf(np.linspace(min(y_values_norm), max(y_values_norm), 100),
                 [y_means[i] - 0.4, y_means[i], y_means[i] + 0.4]) for i in range(6)]

mf = [fuzz.trimf(np.linspace(min(z_values_norm), max(z_values_norm), 100),
                 [z_means[i] - 0.4, z_means[i], z_means[i] + 0.4]) for i in range(9)]
