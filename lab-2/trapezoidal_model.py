...
mx = [fuzz.trapmf(x_values_norm,
                  [x_means[i] - 0.1, x_means[i] - 0.05, x_means[i], x_means[i] + 0.05]) for i in range(6)]

my = [fuzz.trapmf(np.linspace(min(y_values_norm), max(y_values_norm), 100),
                  [y_means[i] - 0.2, y_means[i] - 0.1, y_means[i], y_means[i] + 0.1]) for i in range(6)]

mf = [fuzz.trapmf(np.linspace(min(z_values_norm), max(z_values_norm), 100),
                  [z_means[i] - 0.4, z_means[i] - 0.2, z_means[i], z_means[i] + 0.2]) for i in range(9)]
